...
const mediaPatchSchema = z.object({
  ...
  action: z.enum(["set_header", "remove_header"]),
});
...
/**
 * Convert a Cloudinary URL to serve AVIF format with auto quality.
 * SVG files are returned unchanged (vector format).
 */
function toAvifUrl(cloudinaryUrl: string, isSvg: boolean = false): string {
  if (isSvg || cloudinaryUrl.endsWith('.svg')) {
    return cloudinaryUrl;
  }
  // Add f_avif,q_auto transformation and change extension to .avif
  return cloudinaryUrl
    .replace('/upload/', '/upload/f_avif,q_auto/')
    .replace(/\.(jpg|jpeg|png|webp|gif)$/i, '.avif');
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformAsset = (asset: MediaAssetRecord) => {
      ...
      // Use cloudinary_url (permanent) if available, fallback to file.url (temporary Airtable URL)
      const url = asset.cloudinary_url || file?.url || "";
      return {
        id: asset.id,
        url,
        fileType,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = mediaPostSchema.safeParse(body);

    if (!parsed.success) {
      return NextResponse.json(
        { error: "Ongeldige invoer: type en Cloudinary upload result zijn verplicht" },
        { status: 400 }
      );
    }

    const { type, cloudinaryResult, fileType: originalFileType } = parsed.data;
    ...
    // Get employer to check for existing media
    let employer;
    try {
    ...
    // For logos: generate alt text directly (e.g. "Logo van Bedrijfsnaam")
    // For sfeerbelden: alt_text will be generated by n8n workflow with AI vision
    const altText = type === "logo" 
      ? generateAltText("logo", {
          display_name: employer?.display_name,
          company_name: employer?.company_name,
        })
      : undefined;

    // Determine if this is an SVG (should not be converted to AVIF)
    ...
    try {
      mediaAsset = await createMediaAsset({
        ...
        cloudinary_url: toAvifUrl(cloudinaryResult.secure_url, isSvg),
